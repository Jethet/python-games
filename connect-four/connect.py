...
def winning_move(board, piece):
    ...
    for c in range(COLUMN_COUNT - 3):  # you cannot have 4 from the last 3
        for r in range(ROW_COUNT):
    ...
    # check positively sloped diagonals: determine last possible row
    # for 4 connect
...
def draw_board(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            # draw a blue rectangle, with black row on top
            # (= extra squaresize in r*squaresize+squaresize)
            pygame.draw.rect(
                screen,
                BLUE,
...
RADIUS = int(
    SQUARESIZE / 2 - 5
)  # circles slightly smaller than black square so they are not touching
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # increase turn and use modular to get 0 or 1
            turn += 1
            turn = turn % 2
